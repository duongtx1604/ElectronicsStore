...
namespace ElectronicsStore.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ProductsController(ElectronicsStoreContext context)
        {
            _context = context;
        }
        ...
        {
            var electronicsStoreContext = _context.Product.Include(p => p.Category);
            return View(await electronicsStoreContext.ToListAsync());
        }
        [HttpPost]
        public async Task<IActionResult> Index(int catid, string keywords)
        {
            var electronicsStoreContext = _context.Product.Include(p => p.Category).Where(p => p.Name.Contains(keywords) && p.CategoryId == catid);
            return View(await electronicsStoreContext.ToListAsync());
        }
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            {
                return Problem("Entity set 'ElectronicsStoreContext.Product'  is null.");
            }
            var product = await _context.Product.FindAsync(id);
            if (product != null)
            {
